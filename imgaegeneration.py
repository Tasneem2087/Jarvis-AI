   ...
   #Function to open and display images based on a given prompt
    ...
    headers = {"Authorization": f"Bearer (get_key('.env', 'Hugging FaceAPIKey'))"}

    #Asyne function to send a query to the Hogging Face API
    async def query(payload):
        response = await asyncio.to thread(requests.post, API URL, headers = headers, json = payload)
        ...
        try:
            ...
            if Status == "True":
                ...
                with open(r"Frontend\Files\ImageGeneration.data", "w") as f:
                    f.write("False, False")
                    break   #Exit the loop after processing the request I
